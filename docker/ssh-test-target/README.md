...
A disposable Alpine container running `sshd`, for trying OmnySSH out (host add, key
...
`OMNYSSH_TEST_SSH_HOST`/`_PORT`/`_USER`/`_PASSWORD` point at — this container's
defaults (`127.0.0.1:2222`, `omnyssh`/`omnyssh`) if you don't set them. Start the
container first; these tests are the one place OmnySSH's test suite actually opens a
